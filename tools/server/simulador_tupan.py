...
# ---------------------------------------------------------------------------
DIST: Final[Path] = Path(__file__).resolve().parents[2] / "build"
...
try:
    ...
    def full_cycle(nh, dh, rh, t, ff, ef, ht):
        """Espelho Python do núcleo C++ (fallback): sorção → solenoide →
        vidraria → filtro mineralizante + UV-C → bacia. Mesmas fórmulas,
        mesmas constantes — a paridade é garantida pelos testes."""
        w = sorption_intake(nh, rh, t, ff, ef)
        l = distillation_output(dh, w, ht)
        uv_j = 6.0 * 1800.0                                   # UV-C 6 W × 30 min = 10 800 J
        energy_kj = ((7.5 * nh + 250.0 * dh) * 3600.0 + uv_j) / 1000.0
        ...
        return type("CB", (), {"water_kg_sorbed": w, "distilled_l": l,
                               ...
                               "energy_kj_total": energy_kj,
                               "liters_per_kwh": potavel / (energy_kj / 3600.0) if energy_kj else 0.0})
...
class TupanModel:
    ...
    def __init__(self, env: dict[str, float] | None = None, noise: float = 0.05) -> None:
        ...
        self.noise: float = noise  # ± fração multiplicativa (0 = determinístico)
...
@app.get("/health")
def health() -> Any:
    return jsonify({"status": "ok", "nativo": _NATIVO, "versao": "0.1.0"})
